...
function Sound.init(self, id, source, soundType)
...
function Sound.clean(self, id)
   self.source[id] = nil
end

function Sound.play(self, id, channel, volume, pitch, loop)
   local source
...
function Sound.setVolume(self, channel, volume)
...
function Sound.setPitch(self, channel, pitch)
   assert(Sound.active[channel] ~= nil, "Channel does not exist")
   for k,sound in pairs(Sound.active[channel]) do
...
function Sound.stop(self, channel)
   assert(Sound.active[channel] ~= nil, "Channel does not exist!")
   for k, sound in pairs(Sound.active[channel]) do
      sound:stop()
   end
